...
2. **ニュース本文は外部AIが用意したリサーチファイル（デフォルト: `research.md`）を最優先ソースとする**。引数でファイルパスを指定された場合はそちらを読む。情報が足りないカテゴリだけWebSearchで補完する。
3. WebSearchは「ニュース本文・文脈の補完」専用。価格取得には使わない（ソースによって数値がバラつき信頼できない）。
4. スクショがない場合はユーザーに依頼する（「TradingViewのスクショをいただけますか」）。
5. リサーチファイルにも検索結果にも存在しない事実は絶対に書かない。数値は出典から直接コピーする。
...
### Step 2: ニュース本文リサーチ（リサーチファイル読み込み方式）

**デフォルト動作: 外部AI（Gemini等）が用意したリサーチ結果のマークダウンファイルをReadツールで読み込む。**

1. **引数でファイルパスが指定されているか確認**
   - `/daily-news research.md` のように引数ありで起動された場合 → 指定されたパスのマークダウンを読む
   - 引数なしで起動された場合 → デフォルトの `research.md`（プロジェクトルート）を読む
2. **Readツールでファイル全体を読み込み、内容を解析**
   - 全7カテゴリ（為替/コモディティ/暗号資産/株式指数/地政学/日本テクノロジー/共通ニュース）がカバーされているか確認
   - ニュースの日付・出典URL・数値を抽出してメモ
3. **情報が不足しているカテゴリだけWebSearchで補完**
   - リサーチファイルに無いカテゴリ、または情報が薄いカテゴリのみを対象に最小限の検索を実施
   - 全カテゴリがファイルで十分に網羅されている場合はWebSearchをスキップ
4. **ユーザーからDZH/FISCO/Reuters等のニュースヘッドラインスクショが追加提供されている場合:**
   - それもリサーチ素材として組み合わせる。特に日本語ソース（FISCO, DZH）は日本市場の動きを正確に把握できる

**重要**: リサーチファイルに書かれていない事実を勝手に補間しない。不足情報は必ずWebSearchで裏を取る。
