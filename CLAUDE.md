...
2. **ニュース本文リサーチ**: ユーザーが用意したリサーチ結果のマークダウンファイル（デフォルト: `research.md`、引数で他のパス指定可）をReadツールで読み込む。全7カテゴリを網羅しているか確認し、情報が不足しているカテゴリのみWebSearchで補完する（デフォルトはファイル読み込みであり、WebSearchは補完専用）
